...
from mlak.utils import NoWarnings
...
class DenseParser:
	...
	def make( self_, model, **kwArgs ):
		...
		if not model.layers or ( ( t != Flatten ) and ( t != Dense ) and ( t != Dropout ) ):
			model.add( Flatten( **kwArgs ) )
...
class KerasSolver:
	...
	def train( self_, X, y, solution = None, iterations = 20, batchSize = 64, kerasArgs = {}, **kwArgs ):
		...
		X = X.astype( 'float32' )
		with NoWarnings():
			model.fit( X, y, batch_size = batchSize, epochs = iterations, **kerasArgs )
